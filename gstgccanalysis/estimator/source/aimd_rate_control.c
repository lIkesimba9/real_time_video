...
#include "../header/aimd_rate_control.h"
//#include "razor_log.h"
#include <math.h>
#include <assert.h>
...
static uint32_t aimd_change_bitrate(aimd_rate_controller_t* aimd, uint32_t new_bitrate, rate_control_input_t* input, int64_t cur_ts)
{
	...
	case kRcIncrease:
		...
		aimd->time_last_bitrate_change = cur_ts;
        printf("aimd: kRcIncrease, new bitrate = %u, input->incoming_bitrate = %u, curr_bitrate = %u\n", new_bitrate, input->incoming_bitrate, aimd->curr_rate);
	...
	case kRcDecrease:
        ...
        printf("aimd: RcDecrease, new bitrate = %u, input->incoming_bitrate = %u, curr_bitrate = %u\n", new_bitrate, input->incoming_bitrate, aimd->curr_rate);
